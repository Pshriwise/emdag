...
int main( int argc, char* argv[] )
{
  ...
  if( rays.size() > 0 ){
    ...
    for( unsigned i = 0; i < rays.size(); ++i ){
      ...
      rval = dagmc.ray_fire( vol, ray.p.array(), ray.v.array(), surf, dist, NULL, NULL, 0, 1, trv_stats );
...
#endif
    // added ray orientation
    dagmc.ray_fire(vol, xyz.array(), uvw.array(), surf, dist, NULL, NULL, 0, 1, trv_stats );
